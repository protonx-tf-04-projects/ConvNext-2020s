...
def stage(input, filter_num, num_block, use_downsample=True, model='base', stage_idx=-1):
  ...
  '''
  net = resblock(input = input, filter_num = filter_num, stride = 2 if use_downsample else 1, model=model, 
                stage_idx = stage_idx, block_idx = 1)
    
  for i in range(1, num_block):
    net = resblock(input = net, filter_num = filter_num,stride = 1,model=model, 
